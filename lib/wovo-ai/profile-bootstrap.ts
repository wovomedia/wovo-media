...
export async function ensureProfileForUser(user: AuthUser): Promise<void> {
  ...
  await supabaseServiceRoleRequest("/rest/v1/profiles?on_conflict=user_id", {
    ...
    body: JSON.stringify({ user_id: user.id, email: user.email ?? null, full_name: fullName, avatar_url: avatarUrl, plan: "none", monthly_limit: 0, monthly_used: 0, extra_credits: 0, updated_at: new Date().toISOString() }),
  });
}
