...
export type AiAccessState = { hasAccess: boolean; requiresSubscription: boolean; showPaywall: boolean };
export type AuthAccessState = {
  isAuthenticated: boolean; needsPlan: boolean; hasAppAccess: boolean;
  effectivePlan: "none" | "starter" | "growth" | "pro";
  monthlyPlanCredits: number; showPaywall: boolean;
};

function getMonthlyPlanCredits(plan: string, subscription?: UnifiedSubscriptionResponse | null): number {
  if (typeof subscription?.remaining?.monthly_limit === "number") return subscription.remaining.monthly_limit;
  if (plan === "starter")  return 50;
  if (plan === "growth")   return 150;
...
export function resolveAiAccessState(subscription: UnifiedSubscriptionResponse | null | undefined, userEmail?: string | null): AiAccessState {
  ...
  return { hasAccess, requiresSubscription: !hasAccess, showPaywall: !hasAccess };
}
...
export function getAuthAccessState(params: {
  ...
  const effectivePlan = (adminAccess ? "pro" : (params.subscription?.plan ?? "none")) as AuthAccessState["effectivePlan"];
  ...
  return { isAuthenticated, needsPlan, hasAppAccess, effectivePlan, monthlyPlanCredits, showPaywall: needsPlan };
}
