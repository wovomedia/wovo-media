-- Moves paid video generation onto the canonical tenant usage reservation and
-- credit ledger. Provider submission remains external, while reserve/finalize/
-- release transitions are database-idempotent and tied to one durable job.
...
alter table public.wovo_ai_usage_requests
  ...
  check (feature in ('chat', 'image_visual', 'video', 'website_page', 'product_page', 'code'));

alter table public.video_jobs
  add column if not exists account_id uuid references public.wovo_portal_accounts(id) on delete cascade,
  add column if not exists usage_request_id uuid references public.wovo_ai_usage_requests(id) on delete set null;

update public.video_jobs
set account_id = (result_payload ->> 'previewAccountId')::uuid
where account_id is null
  and result_payload ->> 'previewAccountId' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$';

create index if not exists video_jobs_account_created_idx
  on public.video_jobs(account_id, created_at desc)
  where account_id is not null;
create unique index if not exists video_jobs_usage_request_unique_idx
  on public.video_jobs(usage_request_id)
  where usage_request_id is not null;

drop index if exists public.video_jobs_workspace_preview_unique_idx;
create unique index video_jobs_workspace_preview_unique_idx
...
begin
  ...
  if p_feature not in ('chat', 'image_visual', 'video', 'website_page', 'product_page', 'code') then raise exception 'Invalid AI feature'; end if;
  ...
  from public.wovo_ai_usage_requests
  where account_id = p_account_id and status in ('reserved', 'completed')
    and reserved_at >= pg_catalog.date_trunc('day', now());
...
create or replace function public.wovo_video_create_reserved_job(
  p_job_id uuid,
  p_account_id uuid,
  p_actor_user_id uuid,
  p_prompt text,
...
begin
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('wovo-video-job:' || p_job_id::text, 0));
  select * into v_existing from public.video_jobs where id = p_job_id;
  if found then
    if v_existing.user_id <> p_actor_user_id or v_existing.account_id <> p_account_id then
      raise exception 'Video job identity mismatch';
    end if;
    return v_existing;
  end if;
  if p_prompt is null or pg_catalog.char_length(pg_catalog.btrim(p_prompt)) < 3
     or pg_catalog.char_length(pg_catalog.btrim(p_prompt)) > 6000 then
    raise exception 'Invalid video prompt';
  end if;
  if not exists (
    select 1 from public.wovo_portal_accounts account
    where account.id = p_account_id and account.archived_at is null
  ) then
    raise exception 'Video workspace is unavailable';
  end if;
  if not exists (
    select 1 from public.wovo_portal_accounts account
    where account.id = p_account_id and account.owner_user_id = p_actor_user_id
  ) and not exists (
    select 1 from public.wovo_portal_members member
    where member.account_id = p_account_id and member.user_id = p_actor_user_id and member.active = true
  ) and not exists (
    select 1 from public.wovo_portal_staff staff
    where staff.user_id = p_actor_user_id and staff.active = true
  ) then
  ...
  select * into v_usage from private.wovo_ai_reserve_usage(
    ...
    'video-job:' || p_job_id::text,
    coalesce(p_payload, '{}'::jsonb) || pg_catalog.jsonb_build_object('job_id', p_job_id)
  );

  insert into public.video_jobs(
    id, user_id, account_id, usage_request_id, provider, provider_job_id,
    prompt, status, result_url, result_payload
  ) values (
    p_job_id, p_actor_user_id, p_account_id, v_usage.id, 'fal', null,
    pg_catalog.btrim(p_prompt), 'queued', null,
    coalesce(p_payload, '{}'::jsonb) || pg_catalog.jsonb_build_object(
      ...
      'previewWatermarkRequired', false
    )
...
create or replace function public.wovo_video_fail_job(
  p_job_id uuid,
  p_actor_user_id uuid,
  p_error_code text default 'video_provider_failed'
) returns public.video_jobs
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if not found then raise exception 'Video job not found'; end if;
  if v_job.user_id <> p_actor_user_id then raise exception 'Video job identity mismatch'; end if;
  if v_job.status = 'completed' then return v_job; end if;
  if v_job.usage_request_id is not null then
    perform private.wovo_ai_release_usage(
      v_job.usage_request_id,
      pg_catalog.left(coalesce(p_error_code, 'video_provider_failed'), 120)
    );
  end if;
  update public.video_jobs
  set status = 'failed',
      error = pg_catalog.left(coalesce(p_error_code, 'video_provider_failed'), 120),
      result_payload = result_payload || pg_catalog.jsonb_build_object('meteringStatus', 'released'),
      updated_at = now()
  where id = p_job_id
  returning * into v_job;
  return v_job;
...
create or replace function public.wovo_video_complete_job(
  p_job_id uuid,
  p_actor_user_id uuid,
  p_result_url text,
  p_payload jsonb default '{}'::jsonb
...
begin
  ...
  if v_job.user_id <> p_actor_user_id then raise exception 'Video job identity mismatch'; end if;
  if v_job.status = 'completed' then return v_job; end if;
  if v_job.status = 'failed' then raise exception 'Failed video job cannot be completed'; end if;
  if v_job.usage_request_id is null or v_job.provider_job_id is null then
    raise exception 'Video job is missing metering or provider proof';
  end if;
  select * into v_usage from public.wovo_ai_usage_requests where id = v_job.usage_request_id;
  if not found then raise exception 'Video usage reservation not found'; end if;

  perform private.wovo_ai_finalize_usage(
    v_job.usage_request_id,
    v_usage.estimated_units,
    v_usage.estimated_provider_cost_micros,
    v_job.provider_job_id
  );

  update public.video_jobs
  set status = 'completed', result_url = p_result_url, error = null,
      result_payload = result_payload || coalesce(p_payload, '{}'::jsonb) ||
        pg_catalog.jsonb_build_object('providerCompleted', true, 'meteringStatus', 'completed'),
      updated_at = now()
  where id = p_job_id
  returning * into v_job;
  return v_job;
...
revoke all on function public.wovo_video_create_reserved_job(uuid, uuid, uuid, text, integer, bigint, jsonb)
  from public, anon, authenticated;
revoke all on function public.wovo_video_fail_job(uuid, uuid, text)
  from public, anon, authenticated;
revoke all on function public.wovo_video_complete_job(uuid, uuid, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.wovo_video_create_reserved_job(uuid, uuid, uuid, text, integer, bigint, jsonb)
  to service_role;
grant execute on function public.wovo_video_fail_job(uuid, uuid, text)
  to service_role;
grant execute on function public.wovo_video_complete_job(uuid, uuid, text, jsonb)
  to service_role;

comment on column public.video_jobs.usage_request_id is
  'Canonical reservation finalized only after provider output is stored, or released on durable failure.';
comment on function public.wovo_video_create_reserved_job(uuid, uuid, uuid, text, integer, bigint, jsonb) is
  'Atomically reserves workspace usage and creates one paid fal video job. Service role only.';
