...
export default function SignupPage() {
  ...
  const saveOnboarding = async (token: string) => {
    await fetch("/api/wovo-ai/onboarding", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ full_name: fullName, username, age, gender }),
    });
  };
        ...
        <button
          onClick={async () => {
            ...
            if (session?.access_token) {
              ...
              await saveOnboarding(session.access_token);
              router.push("/wovo-ai");
            } else {
              router.push("/login");
            }
