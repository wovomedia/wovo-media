...
const included = [
  ...
  "Images, videos, cartoons, captions, and campaigns",
  ...
  "Facebook and Instagram connection workflow",
  "WOVO support inbox and billing controls",
];

const packs = [
  { price: "$5", credits: "50 credits", note: "For a small one-time project", badge: "" },
  { price: "$10", credits: "110 credits", note: "10 bonus credits", badge: "POPULAR" },
  { price: "$25", credits: "300 credits", note: "50 bonus credits", badge: "BEST CREDIT VALUE" },
];
  ...
  return <main>
    ...
    <section className="border-b border-[#191714]/10 py-16 sm:py-24">
      <div className="mx-auto max-w-[1180px] px-5 sm:px-8">
        <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#d94326]">One workspace · every tool included</p>
      ...
      <div className="mx-auto max-w-[1180px] px-5 sm:px-8">
        <div className="overflow-hidden rounded-[28px] border border-[#191714]/15 bg-[#fffdf8] shadow-[0_30px_90px_rgba(25,23,20,.09)]">
          ...
          <div className="grid lg:grid-cols-4">
            {periods.map((period, index) => <article key={period.name} className={`relative p-5 sm:p-7 ${index ? "border-t border-[#191714]/10 lg:border-l lg:border-t-0" : ""} ${period.badge === "BEST DEAL" ? "bg-[#f8eee5]" : ""}`}>
              ...
              <p className="mt-1 text-xs leading-5 text-[#756e64]">{period.savings} · 100 credits every 7 days</p>
            </article>)}
          </div>
          <div className="grid gap-8 border-t border-[#191714]/10 p-5 sm:p-8 lg:grid-cols-[1.2fr_.8fr]">
            <div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#d94326]">Included with every billing period</p><ul className="mt-4 grid gap-x-7 sm:grid-cols-2">{included.map((item) => <li key={item} className="flex min-h-12 items-center gap-3 border-b border-[#191714]/10 py-2 text-sm"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#f2563d]" />{item}</li>)}</ul></div>
        ...
        <section className="mt-10 rounded-[28px] bg-[#191714] p-5 text-white sm:p-8">
          ...
          <div className="mt-7 grid gap-3 md:grid-cols-3">{packs.map((pack) => <article key={pack.price} className="relative rounded-2xl border border-white/15 bg-white/[.06] p-5">
            ...
            <strong className="text-4xl font-medium tracking-[-.04em]">{pack.price}</strong><p className="mt-4 text-lg font-bold">{pack.credits}</p><p className="mt-1 text-xs text-white/55">{pack.note}</p>
          </article>)}</div>
