...
import { defaultPublicMode, estimatePublicCredits, publicModeLabel, PUBLIC_MODEL_CATALOG, type PublicCreationType, type PublicGenerationMode } from "@/lib/ai/public-model-catalog";
...
const TYPES: Array<{ id: PublicCreationType; label: string; icon: IconName }> = [
...
export default function WovoCreateExperience() {
  ...
  const [type, setType] = useState<PublicCreationType>("image");
  ...
  function chooseType(next: PublicCreationType) {
    setType(next);
  ...
  async function generate() {
    ...
    setGenerateError("");
    const intent = { prompt: prompt.trim(), type, modelId: selectedModel.id, ratio: activeRatio, mode, outputCount: effectiveOutputCount, durationSeconds: type === "audio" ? audioDuration : null, durationLabel: duration, resolution: resolutionLabel, audio: audioLabel, credits, referenceName: referenceName || null, referenceData, createdAt: new Date().toISOString() };
    ...
    <main className="wm-v2-root min-h-screen bg-[#0b0b0c] text-[#f7f4ee]">
      <div className="grid min-h-screen lg:grid-cols-[238px_minmax(0,1fr)]">
        <aside className={`${mobileOpen ? "fixed inset-0 z-50 flex" : "hidden"} flex-col border-r border-white/10 bg-[#101011] p-4 lg:sticky lg:top-0 lg:flex lg:h-screen`}>
          ...
          <div className="mt-auto rounded-2xl border border-white/10 bg-white/[.035] p-3.5"><div className="flex items-center justify-between text-xs"><span className="text-white/55">Starter credits</span><strong>10</strong></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full w-1/4 rounded-full bg-[#f05a3a]" /></div><Link href="/pricing" className="mt-4 inline-flex min-h-10 w-full items-center justify-center rounded-xl bg-white text-xs font-bold text-black">View pricing</Link></div>
        ...
        <section className="min-w-0">
          ...
          <div className="mx-auto max-w-[1180px] px-4 pb-16 pt-10 sm:px-7 sm:pt-16 lg:px-10 lg:pt-20">
            <section className="mx-auto max-w-[900px] text-center"><span className="inline-flex items-center gap-2 rounded-full border border-[#f05a3a]/25 bg-[#f05a3a]/10 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[.13em] text-[#ff8c70]"><span className="h-1.5 w-1.5 rounded-full bg-[#f05a3a]" />10 free credits · no card</span><h1 className="mt-6 text-[clamp(2.6rem,6vw,5rem)] font-medium leading-[.96] tracking-[-.055em]">What do you want to make?</h1><p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-white/50">Type it or say it. Adam can route the job, show the exact credit cost, and keep the result in your WOVO workspace.</p></section>

            <section className="mx-auto mt-10 max-w-[960px]">
              <div className="flex flex-wrap justify-center gap-2" role="tablist" aria-label="Creation type">{TYPES.map((item) => <button key={item.id} onClick={() => chooseType(item.id)} className={`inline-flex min-h-11 items-center gap-2 rounded-full px-4 text-sm font-semibold transition ${type === item.id ? "bg-white text-black" : "border border-white/10 bg-white/[.03] text-white/55 hover:text-white"}`} aria-pressed={type === item.id}><Icon name={item.icon} className="h-4 w-4" />{item.label}</button>)}</div>
              <div className="mt-5 overflow-hidden rounded-[28px] border border-white/14 bg-[#151516] shadow-[0_32px_100px_rgba(0,0,0,.45)] focus-within:border-[#f05a3a]/55">
                <textarea id="create-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={`Describe the ${type} you want to create…`} className="min-h-40 w-full resize-none bg-transparent px-5 pb-3 pt-6 text-lg leading-8 text-white outline-none placeholder:text-white/28 sm:min-h-48 sm:px-7 sm:text-xl" />
                <div className="flex flex-col gap-3 border-t border-white/10 p-3.5 sm:flex-row sm:items-center sm:p-4">
                  ...
                  <div className="ml-auto flex items-center justify-end gap-3"><div className="text-right"><p className="text-sm font-bold">{credits} credits</p><p className="text-[10px] text-white/38">Exact estimate</p></div><button onClick={() => void generate()} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#f05a3a] px-5 text-sm font-black text-[#140b08] shadow-[0_10px_35px_rgba(240,90,58,.28)] hover:bg-[#ff7659]">Generate <Icon name="arrow" className="h-4 w-4" /></button></div>
                </div>
              </div>
              ...
              <div className="mt-3 overflow-hidden rounded-2xl border border-white/10 bg-black/25"><div className="grid grid-cols-2 gap-px bg-white/10 sm:grid-cols-4 lg:grid-cols-7">{[["Model", selectedModel.name], ["Mode", publicModeLabel(mode)], ["Duration", duration], ["Resolution", resolutionLabel], ["Audio", audioLabel], ["Outputs", String(effectiveOutputCount)], ["Total", `${credits} credits`]].map(([label, value]) => <div key={label} className="min-w-0 bg-[#111112] px-3 py-3"><p className="text-[9px] font-bold uppercase tracking-[.14em] text-white/30">{label}</p><p className={`mt-1 truncate text-xs font-semibold ${label === "Total" ? "text-[#ff8c70]" : "text-white/68"}`} title={value}>{value}</p></div>)}</div></div>
