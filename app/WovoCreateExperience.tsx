...
const EXPLORE = [
  { title: "Restaurant launch film", type: "Video", model: "Kling 3.0 Pro", image: "/wovo-creator-hero.png", position: "74% 48%" },
...
export default function WovoCreateExperience({ availability }: { availability: CreationAvailability }) {
  ...
  const audioLabel = type === "audio" ? "Audio output" : type === "video" || type === "cartoon" ? "On · native audio when model supports it" : "Not applicable";
      ...
      <div className="grid min-h-screen lg:grid-cols-[238px_minmax(0,1fr)]">
        <aside className={`${mobileOpen ? "fixed inset-0 z-50 flex" : "hidden"} flex-col border-r border-white/10 bg-[#101011] p-4 lg:sticky lg:top-0 lg:flex lg:h-screen`}>
          <div className="flex min-h-12 items-center justify-between px-2">
            <Link href="/" className="inline-flex items-center gap-2.5" aria-label="WOVO home"><img src="/images/brand/wovo-glow-icon.svg" alt="" width="36" height="36" className="h-9 w-9 object-contain" /><span className="text-[21px] font-black tracking-[-.075em]">WOVO</span><span className="rounded-full border border-white/20 px-2 py-1 text-[8px] font-bold uppercase tracking-[.2em] text-white/55">Studio</span></Link>
