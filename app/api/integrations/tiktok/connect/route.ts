...
export async function POST(request: Request) {
  try {
    ...
    if (!clientKey || !getEnv("TIKTOK_CLIENT_SECRET") || !socialEncryptionConfigured()) {
      return NextResponse.json({ error: "TikTok connection is not configured for this deployment." }, { status: 503 });
    }
    const state = randomBytes(32).toString("hex");
    const redirectUri = socialRedirectUrl(new URL(request.url).origin, "tiktok");
