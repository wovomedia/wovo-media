...
export async function GET(request: Request) {
  try {
    ...
    const accessState = getAuthAccessState({ user: { id: user.id, email: user.email ?? null }, subscription: status });
    ...
    return NextResponse.json({ error: "Failed to load subscription.", status: "inactive", plan: "none", remaining: { monthly_limit: 0, monthly_used: 0, extra_credits: 0, credits_remaining: 0 }, has_access: false, requires_subscription: true, can_generate: false, admin_access: false, has_app_access: false, needs_plan: true, show_paywall: true, effective_plan: "none", monthly_plan_credits: 0 }, { status: 500 });
  }
}
