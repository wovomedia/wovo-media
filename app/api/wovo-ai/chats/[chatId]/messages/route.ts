...
export async function GET(request: Request, { params }: { params: Promise<{ chatId: string }> }) {
  try {
    ...
    const messages = await supabaseServiceRoleRequest<MessageRow[]>(
      `/rest/v1/messages?select=id,role,content,created_at&chat_id=eq.${encodeChatId(chatId)}&user_id=eq.${user.id}&order=created_at.asc`,
    );
