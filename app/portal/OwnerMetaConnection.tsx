...
export default function OwnerMetaConnection() {
  ...
  async function post(url: string, payload: Record<string, unknown>) {
    ...
    try {
      ...
      const result = await response.json() as { error?: string; url?: string; published?: boolean };
      if (!response.ok) throw new Error(result.error || "Meta request failed.");
      if (result.url) { window.location.href = result.url; return; }
      setMessage(result.published
        ? (payload.action === "publish_image_test" ? "Meta confirmed the branded image on Facebook and Instagram." : "Meta confirmed the WOVO Page test post.")
  ...
  return <section className="rounded-2xl border border-[#191714]/10 bg-[#fffdf8] p-5 shadow-[0_18px_55px_rgba(25,23,20,.07)] sm:p-6">
    ...
    {!data.connection ? <div className="mt-5"><p className="text-sm text-[#655f56]">{ready ? "Connect the WOVO-managed Facebook Page and its linked Instagram professional account." : "Meta stays blocked until the app credentials, feature gate, and token encryption key are configured."}</p>{ready ? <button disabled={busy === "connect"} className={`${button} mt-4`} onClick={() => void post("/api/integrations/meta/connect", {})}>Connect WOVO Meta accounts</button> : null}<p className="mt-3 text-xs text-[#81796f]">OAuth callback: {data.runtime.redirectUrl}</p></div> : <div className="mt-5 grid gap-5 lg:grid-cols-2">
      ...
      {!data.connection.killSwitch ? <div className="rounded-xl border border-[#f05a3a]/25 p-4 lg:col-span-2"><p className="font-bold">Automatic image publishing</p><p className="mt-2 text-sm leading-6 text-[#655f56]">With <strong>Scheduled automatic posts</strong> selected, WOVO queues one unique original branded image and factual caption at 9:00 AM, 1:00 PM, and 6:00 PM America/Chicago, then publishes each once to the connected Facebook Page and Instagram account. Per-slot idempotency keys prevent duplicates; the kill switch stops both destinations without a catch-up burst.</p>{data.connection.actionPolicy === "scheduled_auto_publish" ? <button disabled={busy === "publish_image_test"} className={`${button} mt-4 bg-[#f05a3a] text-[#191714]`} onClick={() => void post("/api/integrations/meta/publish", { action: "publish_image_test", confirmed: true })}>Publish today’s image test to both</button> : <p className="mt-3 text-xs font-bold uppercase tracking-[.1em] text-[#8a5d17]">Select Scheduled automatic posts and save to run the first image test.</p>}</div> : null}
    </div>}
  </section>;
}
