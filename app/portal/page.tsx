    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f3efe6] text-[#191714]">
      ...
      <div className="mx-auto grid max-w-[1800px] gap-6 px-4 py-6 pb-28 sm:px-6 sm:pb-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <section className="min-w-0">
          ...
          {tab === "queue" && account ? <Queue account={account} items={content} drafts={workflowDrafts} assets={assets} creditBalance={snapshot.creditAccounts.find((item) => item.account_id === account.id)?.balance ?? 0} paid={isPaid} staff={snapshot.mode === "staff"} aiConfigured={snapshot.setup.aiConfigured} busy={busy} onAction={action} authedFetch={authedFetch} reload={load} setError={setError} setNotice={setNotice} /> : null}
...
function Queue({ account, items, drafts, assets, creditBalance, paid, staff, aiConfigured, busy, onAction, authedFetch, reload, setError, setNotice }: { account: PortalAccount; items: PortalContentItem[]; drafts: PortalSnapshot["workflowDrafts"]; assets: PortalSnapshot["assets"]; creditBalance: number; paid: boolean; staff: boolean; aiConfigured: boolean; busy: string; onAction: (payload: Record<string, unknown>, success: string) => Promise<unknown>; authedFetch: (url: string, init?: RequestInit) => Promise<Response>; reload: () => Promise<void>; setError: (value: string) => void; setNotice: (value: string) => void }) {
  ...
  return (
    <div className="space-y-5">
      <CreatorWorkbench account={account} items={items} drafts={drafts} assets={assets} creditBalance={creditBalance} paid={paid} aiConfigured={aiConfigured} busy={busy} onAction={onAction} authedFetch={authedFetch} reload={reload} setError={setError} setNotice={setNotice} />
