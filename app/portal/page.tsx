...
const studioFieldClass = "mt-1 min-h-11 w-full rounded-xl border border-white/10 bg-[#24211f] px-3 text-sm text-white outline-none transition [color-scheme:dark] focus:border-[#f05a3a] focus:ring-2 focus:ring-[#f05a3a]/20";
...
export default function PortalPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f3efe6] text-[#191714]">
      ...
      <div className="mx-auto grid max-w-[1800px] gap-6 px-4 py-6 pb-28 sm:px-6 sm:pb-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside>
          <nav className="hidden space-y-1 sm:block" aria-label="Portal">
            {tabs.map((item) => <button key={item.value} onClick={() => setTab(item.value)} className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-medium transition ${tab === item.value ? "bg-[#191714] text-white shadow-sm" : "text-[#655f56] hover:bg-[#191714]/[.04] hover:text-[#191714]"}`}><span className={`inline-flex h-7 w-7 items-center justify-center rounded-lg text-[10px] font-black ${tab === item.value ? "bg-[#f05a3a] text-[#191714]" : "bg-[#191714]/[.06]"}`}>{item.mark}</span>{item.label}</button>)}
          </nav>
        ...
        <section className="min-w-0">
          ...
          {tab === "queue" && account ? <Queue account={account} items={content} drafts={workflowDrafts} assets={assets} creditBalance={snapshot.creditAccounts.find((item) => item.account_id === account.id)?.balance ?? 0} ownerExempt={snapshot.mode === "staff" && snapshot.staffRole === "owner"} paid={isPaid} staff={snapshot.mode === "staff"} aiConfigured={snapshot.setup.aiConfigured} busy={busy} onAction={action} authedFetch={authedFetch} reload={load} setError={setError} setNotice={setNotice} /> : null}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-6 border-t border-[#191714]/10 bg-[#fffdf8]/95 px-1 pb-2 pt-1.5 shadow-[0_-16px_42px_rgba(25,23,20,.1)] backdrop-blur-xl sm:hidden" aria-label="Mobile workspace">
        {tabs.map((item) => <button key={item.value} type="button" onClick={() => { setTab(item.value); window.scrollTo({ top: 0, behavior: "smooth" }); }} aria-current={tab === item.value ? "page" : undefined} className={`min-h-14 rounded-xl px-1 text-[10px] font-bold leading-3 transition ${tab === item.value ? "bg-[#f05a3a]/15 text-[#a9341f]" : "text-[#655f56]"}`}><span className={`mx-auto mb-1 inline-flex h-5 w-5 items-center justify-center rounded-md text-[9px] ${tab === item.value ? "bg-[#f05a3a] text-[#191714]" : "bg-[#191714]/10"}`}>{item.mark}</span><span className="block">{item.label}</span></button>)}
      </nav>
...
function CreatorWorkbench({ account, items, drafts, assets, creditBalance, ownerExempt, aiConfigured, busy, onAction, authedFetch, reload, setError, setNotice }: {
  ...
  const recentOutputs = [
    ...
    ...drafts.slice(0, 4).map((draft) => ({ id: draft.id, title: draft.title, kind: draft.workflow_type.replaceAll("_", " "), status: draft.status, date: draft.created_at })),
  ].sort((a, b) => Date.parse(b.date) - Date.parse(a.date)).slice(0, 6);

  useEffect(() => {
    let active = true;
    void authedFetch(`/api/integrations/meta/status?accountId=${encodeURIComponent(account.id)}`)
      .then(async (response) => response.ok ? response.json() as Promise<{ connection?: { status: string; pageName: string; instagramUsername: string | null } }> : null)
      .then((payload) => { if (active && payload?.connection?.status === "healthy") setMetaDestination({ pageName: payload.connection.pageName, instagramUsername: payload.connection.instagramUsername }); })
      .catch(() => null);
    return () => { active = false; };
  ...
  const channelChoices = mode === "website"
    ...
    : mode === "video" || mode === "episode"
      ? [{ value: "instagram", label: metaDestination?.instagramUsername ? `Instagram · @${metaDestination.instagramUsername}` : "Instagram" }, { value: "facebook", label: metaDestination?.pageName ? `Facebook · ${metaDestination.pageName}` : "Facebook" }, { value: "tiktok", label: "TikTok" }, { value: "youtube", label: "YouTube" }]
      : [{ value: "instagram", label: metaDestination?.instagramUsername ? `Instagram · @${metaDestination.instagramUsername}` : "Instagram" }, { value: "facebook", label: metaDestination?.pageName ? `Facebook · ${metaDestination.pageName}` : "Facebook" }, { value: "linkedin", label: "LinkedIn" }];
  ...
  return (
    <section className="overflow-hidden rounded-[28px] border border-[#191714]/15 bg-[#fffdf8] shadow-[0_28px_90px_rgba(25,23,20,.16)]">
      <header className="flex flex-col gap-4 border-b border-[#191714]/10 px-4 py-5 sm:px-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#d94326]">WOVO Creative Studio</p>
          <h1 className="mt-2 text-3xl font-medium tracking-[-.04em] sm:text-[2.7rem]">Describe what you want Adam to create.</h1>
        ...
        <div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-[#191714]/10 bg-[#f7f2e9] lg:min-w-72">
          <div className="px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#756e64]">Credits</p><p className="mt-1 text-lg font-semibold">{ownerExempt ? "∞" : creditBalance}</p>{ownerExempt ? <p className="text-[9px] font-bold uppercase tracking-[.1em] text-[#a9341f]">Owner exempt</p> : null}</div>
      ...
      <div className="bg-[#171513] text-white">
        <nav className="flex gap-1 overflow-x-auto border-b border-white/10 px-3 py-2 sm:px-5" role="tablist" aria-label="Creation mode">
          {CREATOR_MODES.map((item) => <button key={item.value} type="button" role="tab" aria-selected={mode === item.value} onClick={() => selectMode(item.value)} className={`min-h-10 shrink-0 rounded-lg px-3 text-sm font-semibold transition ${mode === item.value ? "bg-[#f05a3a] text-[#191714] shadow-[0_8px_24px_rgba(240,90,58,.22)]" : "text-white/60 hover:bg-white/[.06] hover:text-white"}`}>{item.label}</button>)}
        </nav>
        ...
        <form onSubmit={(event) => void submit(event)} className="p-3 sm:p-5">
          <div className="grid gap-4 xl:grid-cols-[200px_minmax(0,1fr)_240px]">
            ...
            <div className="order-1 min-w-0 xl:order-2">
              ...
              <button type="button" aria-expanded={advanced} onClick={() => setAdvanced((value) => !value)} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-xl px-2 text-sm font-semibold text-[#ff9b82] hover:text-white"><span aria-hidden="true">{advanced ? "−" : "+"}</span>{advanced ? "Hide project details" : "Add project details"}</button>
              {advanced ? <div className="mt-2 grid gap-3 rounded-2xl border border-white/10 bg-[#201d1b] p-4 sm:grid-cols-2"><label className="text-xs font-semibold text-white/65">Internal title<input name="title" maxLength={180} className={studioFieldClass} placeholder="Optional project title" /></label><label className="text-xs font-semibold text-white/65">Style / motion<select name="style" className={studioFieldClass}><option value="editorial">Editorial and composed</option><option value="energetic">Energetic movement</option><option value="cinematic">Cinematic and restrained</option><option value="playful">Playful character motion</option><option value="product">Product-focused</option></select></label><label className="text-xs font-semibold text-white/65">Hashtags<input name="hashtags" maxLength={300} className={studioFieldClass} placeholder="#marketing #localbusiness" /></label><label className="text-xs font-semibold text-white/65">Schedule<input name="scheduledFor" type="datetime-local" className={studioFieldClass} /></label><label className="text-xs font-semibold text-white/65">Cadence<input name="cadence" maxLength={80} className={studioFieldClass} placeholder="One-time or weekly" /></label></div> : null}
              {mode === "episode" ? <div className="mt-3 grid gap-3 rounded-2xl border border-[#f05a3a]/25 bg-[#f05a3a]/8 p-4 sm:grid-cols-2"><label className="text-xs font-semibold text-white/65">Character name<input name="characterName" maxLength={100} className={studioFieldClass} placeholder="Mrs. Hellen, Boots…" /></label><label className="text-xs font-semibold text-white/65">Personality<input name="characterPersonality" maxLength={300} className={studioFieldClass} placeholder="Warm, funny, direct…" /></label><label className="text-xs font-semibold text-white/65 sm:col-span-2">Episode goal<textarea name="episodeGoal" maxLength={1200} className={`${studioFieldClass} min-h-24 py-3`} placeholder="What happens, what viewers learn, and how the episode ends" /></label><div className="sm:col-span-2"><p className="text-xs font-semibold text-white/65">Approved logo or character reference</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><label className="cursor-pointer"><input type="radio" name="characterAssetId" value="" defaultChecked className="peer sr-only" /><span className="flex min-h-11 items-center rounded-xl border border-white/10 px-3 text-xs text-white/60 peer-checked:border-[#f05a3a] peer-checked:text-[#ff9b82]">No reference selected</span></label>{imageAssets.map((asset) => <label key={asset.id} className="cursor-pointer"><input type="radio" name="characterAssetId" value={asset.id} className="peer sr-only" /><span className="flex min-h-11 items-center truncate rounded-xl border border-white/10 px-3 text-xs text-white/60 peer-checked:border-[#f05a3a] peer-checked:text-[#ff9b82]">{asset.file_name}</span></label>)}</div></div></div> : null}
              {mode === "video" ? <div className="mt-3 rounded-2xl border border-[#f05a3a]/25 bg-[#f05a3a]/8 p-4"><div className="flex items-start gap-3"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#f05a3a]" /><div><p className="text-sm font-semibold">Storyboard workflow active</p><p className="mt-1 text-xs leading-5 text-white/55">Build the exact shot brief and approved reference package before spending credits on a provider render.</p></div></div><fieldset className="mt-4"><legend className="text-xs font-semibold text-white/65">Duration</legend><div className="mt-2 flex gap-2">{[4,8,12].map((seconds) => <label key={seconds} className="cursor-pointer"><input type="radio" name="duration" value={seconds} defaultChecked={seconds === 8} className="peer sr-only" /><span className="inline-flex min-h-10 items-center rounded-xl border border-white/10 bg-white/[.04] px-4 text-xs font-bold text-white/60 peer-checked:border-[#f05a3a] peer-checked:bg-[#f05a3a]/15 peer-checked:text-[#ff9b82]">{seconds}s</span></label>)}</div></fieldset><fieldset className="mt-4"><legend className="text-xs font-semibold text-white/65">Private start frame</legend><div className="mt-2 grid gap-2 sm:grid-cols-2"><label className="cursor-pointer"><input type="radio" name="startFrameAssetId" value="" defaultChecked className="peer sr-only" /><span className="flex min-h-11 items-center rounded-xl border border-white/10 bg-white/[.04] px-3 text-xs text-white/60 peer-checked:border-[#f05a3a] peer-checked:text-[#ff9b82]">No start frame</span></label>{imageAssets.map((asset) => <label key={asset.id} className="cursor-pointer"><input type="radio" name="startFrameAssetId" value={asset.id} className="peer sr-only" /><span className="flex min-h-11 items-center truncate rounded-xl border border-white/10 bg-white/[.04] px-3 text-xs text-white/60 peer-checked:border-[#f05a3a] peer-checked:text-[#ff9b82]">{asset.file_name}</span></label>)}</div></fieldset></div> : null}
              <div className="mt-4 space-y-2 rounded-2xl border border-white/10 bg-[#201d1b] p-4"><label className="flex min-h-11 items-start gap-2 text-sm text-white/75"><input required name="rightsConfirmed" type="checkbox" className="mt-1 accent-[#f05a3a]" /><span>I own or have permission to use every supplied reference and business fact.</span></label>{["episode", "video"].includes(mode) ? <><label className="flex min-h-11 items-start gap-2 text-sm text-white/75"><input required name="peopleConsentConfirmed" type="checkbox" className="mt-1 accent-[#f05a3a]" /><span>I have permission for every recognizable person or likeness.</span></label><label className="flex min-h-11 items-start gap-2 text-sm text-white/75"><input required name="voiceConsentConfirmed" type="checkbox" className="mt-1 accent-[#f05a3a]" /><span>I have permission for every referenced voice; no impersonation.</span></label></> : null}</div>
            </div>

            <aside className="order-3 rounded-2xl border border-white/10 bg-[#201d1b] p-3" aria-label="Output gallery">
              <div className="flex items-center justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#ff8c70]">Output gallery</p><p className="mt-1 text-xs text-white/40">Saved in this workspace</p></div><span className="text-xs font-semibold text-white/55">{recentOutputs.length}</span></div>
              <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-1">{recentOutputs.slice(0, 4).map((item) => <article key={item.id} className="group rounded-xl border border-white/8 bg-white/[.035] p-3 transition hover:border-[#f05a3a]/35"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-[.1em] text-white/35">{item.kind}</span><span className="h-1.5 w-1.5 rounded-full bg-[#f05a3a]" /></div><h3 className="mt-2 line-clamp-2 text-sm font-semibold leading-5">{item.title}</h3><p className="mt-3 text-[11px] capitalize text-[#ff9b82]">{item.status.replaceAll("_", " ")}</p></article>)}{!recentOutputs.length ? <div className="rounded-xl border border-dashed border-white/12 p-5 text-center"><p className="text-sm font-medium">No saved output yet</p><p className="mt-2 text-xs leading-5 text-white/40">Your first reviewable draft will appear here.</p></div> : null}</div>
            </aside>
          </div>
        ...
        {mode === "video" ? <form onSubmit={(event) => void uploadFrame(event)} className="border-t border-white/10 bg-[#201d1b] p-4 sm:p-5"><div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-sm font-semibold">Add a private reference frame</p><p className="mt-1 text-xs leading-5 text-white/45">JPG, PNG, or WebP up to 10 MB. It stays private to this workspace.</p><input required name="file" type="file" accept="image/jpeg,image/png,image/webp" className="mt-3 block max-w-full text-sm text-white/55 file:mr-3 file:rounded-xl file:border-0 file:bg-[#f05a3a] file:px-4 file:py-2.5 file:font-semibold file:text-[#191714]" /><div className="mt-3 flex flex-col gap-2 sm:flex-row"><label className="flex min-h-11 items-center gap-2 text-xs text-white/65"><input required name="rightsConfirmed" type="checkbox" className="accent-[#f05a3a]" />I own or can use this frame.</label><label className="flex min-h-11 items-center gap-2 text-xs text-white/65"><input required name="peopleConsentConfirmed" type="checkbox" className="accent-[#f05a3a]" />People and likeness consent is confirmed.</label></div></div><button disabled={uploading} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/15 bg-white/[.06] px-4 text-sm font-semibold text-white hover:border-[#f05a3a]/45">{uploading ? "Uploading…" : "Upload privately"}</button></div></form> : null}

        <footer className="border-t border-white/10 bg-[#11100f] p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#ff8c70]">Project workbench</p><h2 className="mt-1 text-lg font-medium">Drafts moving through review</h2></div><span className="text-xs text-white/40">{items.length + drafts.length} saved</span></div><div className="mt-4 flex gap-3 overflow-x-auto pb-1">{recentOutputs.map((item) => <article key={item.id} className="min-w-56 rounded-xl border border-white/10 bg-white/[.045] p-3"><div className="h-1 w-10 rounded-full bg-[#f05a3a]" /><p className="mt-3 text-[10px] font-bold uppercase tracking-[.12em] text-white/35">{item.kind}</p><h3 className="mt-2 line-clamp-2 text-sm font-semibold">{item.title}</h3><div className="mt-3 flex items-center justify-between gap-2"><p className="text-[11px] capitalize text-[#ff9b82]">{item.status.replaceAll("_", " ")}</p><p className="text-[10px] text-white/30">{formatDate(item.date, { month: "short", day: "numeric" })}</p></div></article>)}{!recentOutputs.length ? <p className="w-full rounded-xl border border-dashed border-white/12 p-5 text-center text-sm text-white/40">Create the first private draft to start this workbench.</p> : null}</div></footer>
      </div>
    </section>
  );
}

function Queue({ account, items, drafts, assets, creditBalance, ownerExempt, paid, staff, aiConfigured, busy, onAction, authedFetch, reload, setError, setNotice }: { account: PortalAccount; items: PortalContentItem[]; drafts: PortalSnapshot["workflowDrafts"]; assets: PortalSnapshot["assets"]; creditBalance: number; ownerExempt: boolean; paid: boolean; staff: boolean; aiConfigured: boolean; busy: string; onAction: (payload: Record<string, unknown>, success: string) => Promise<unknown>; authedFetch: (url: string, init?: RequestInit) => Promise<Response>; reload: () => Promise<void>; setError: (value: string) => void; setNotice: (value: string) => void }) {
  ...
  return (
    <div className="space-y-5">
      <CreatorWorkbench account={account} items={items} drafts={drafts} assets={assets} creditBalance={creditBalance} ownerExempt={ownerExempt} paid={paid} aiConfigured={aiConfigured} busy={busy} onAction={onAction} authedFetch={authedFetch} reload={reload} setError={setError} setNotice={setNotice} />
