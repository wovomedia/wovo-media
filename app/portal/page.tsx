  ...
  return (
    <div className="space-y-5">
      ...
      <section className={cardClass}>
        <div className="flex items-center justify-between"><h2 className="text-lg font-semibold">Scheduled posts</h2><span className="text-sm text-[#7a7369]">{items.length} total</span></div>
        <div className="mt-4 grid gap-4">
          {items.map((item) => <article key={item.id} className="rounded-2xl border border-[#191714]/10 bg-[#191714]/[.035] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{item.title}</p><p className="mt-1 text-xs capitalize text-[#7a7369]">{item.platform.replace("_", " ")} · {formatDate(item.scheduled_for)}</p></div><StatusPill status={item.status} /></div><p className="mt-4 whitespace-pre-wrap text-sm leading-6 text-[#5f574e]">{item.caption}</p><div className="mt-4 flex flex-wrap gap-2"><button className={secondaryButton} onClick={() => void navigator.clipboard.writeText(item.caption)}>Copy caption</button>{!staff && ["client_review","revision_requested"].includes(item.status) ? <><button className={primaryButton} onClick={() => void onAction({ action: "update_content", accountId: account.id, contentId: item.id, status: "approved" }, "Post approved and the WOVO team was notified.")}>Approve</button><button className={secondaryButton} onClick={() => void onAction({ action: "update_content", accountId: account.id, contentId: item.id, status: "revision_requested", feedback: "Please revise this post." }, "Revision requested.")}>Request revision</button></> : null}{staff && item.status !== "manual_posted" ? <button className={primaryButton} onClick={() => void onAction({ action: "update_content", accountId: account.id, contentId: item.id, status: "manual_posted" }, "Post marked as manually published.")}>Mark posted</button> : null}</div></article>)}
