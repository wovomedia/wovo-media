        ...
        <form onSubmit={(event) => void submit(event)} className="p-3 sm:p-5">
          <div className="grid gap-4 xl:grid-cols-[200px_minmax(0,1fr)_240px]">
            ...
            <div className="order-1 min-w-0 xl:order-2">
              ...
              {mode === "video" ? <div className="mt-3 rounded-2xl border border-[#f05a3a]/25 bg-[#f05a3a]/8 p-4"><div className="flex items-start gap-3"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#f05a3a]" /><div><p className="text-sm font-semibold">Storyboard workflow active</p><p className="mt-1 text-xs leading-5 text-white/55">Video rendering is not exposed because the provider, metering, private storage, moderation, and refund path has not passed end to end.</p></div></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold text-white/65">Duration<select name="duration" defaultValue="8" className={studioFieldClass}><option value="4">4 seconds</option><option value="8">8 seconds</option><option value="12">12 seconds</option></select></label><label className="text-xs font-semibold text-white/65">Private start frame<select name="startFrameAssetId" className={studioFieldClass}><option value="">No start frame</option>{imageAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.file_name}</option>)}</select></label></div><p className="mt-3 text-xs text-white/40">End-frame control stays hidden until a verified provider supports it.</p></div> : null}
