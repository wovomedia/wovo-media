...
function OperatorPricing({ options, busy, onSelect }: { options: OperatorSnapshot["billingOptions"]; busy: string; onSelect: (frequency: string) => void }) {
  if (!options.length) return <section className={surface}><p className="font-bold">AI Operator billing is not available yet.</p><p className="mt-2 text-sm leading-6 text-[#655f56]">Setup can be saved, but purchase remains hidden until all three server-allowlisted Stripe prices pass amount, currency, interval, webhook, and cancellation verification.</p></section>;
