...
type ConnectionStatus = {
  ...
  connections?: Array<{ id: string; status: string; pageName: string; instagramUsername: string | null; actionPolicy: string; killSwitch: boolean }>;
};
...
export default function ClientMetaConnection({ accountId }: { accountId: string }) {
  ...
  async function enableApprovedScheduling() {
    setBusy(true); setError("");
    try {
      const response = await request("/api/integrations/meta/publish", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "update_policy", accountId, actionPolicy: "approve_each", killSwitch: false }) });
      const payload = await response.json() as { error?: string };
      if (!response.ok) throw new Error(payload.error || "Approved scheduling could not be enabled.");
      await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Approved scheduling could not be enabled."); }
    finally { setBusy(false); }
  }

  const connections = data?.connections ?? (data?.connection ? [{ ...data.connection, id: data.connection.id ?? "primary" }] : []);
  ...
  return <section className="overflow-hidden rounded-[24px] border border-[#191714]/10 bg-[#fffdf8] shadow-[0_18px_55px_rgba(25,23,20,.08)]">
    <div className="grid lg:grid-cols-[.78fr_1.22fr]">
      ...
      <div className="p-6 sm:p-7">
        ...
        {connected && connection ? <div className="mt-5 space-y-3">{connections.map((item) => <article key={item.id} className="rounded-2xl border border-[#191714]/10 bg-[#f7f2e9] p-5"><p className="text-lg font-bold">{item.pageName}</p><p className="mt-1 text-sm text-[#655f56]">{item.instagramUsername ? `Instagram @${item.instagramUsername}` : "Facebook Page only"}</p><div className="mt-3 grid gap-2 text-xs text-[#756e64] sm:grid-cols-2"><p><strong className="block text-[#191714]">Publishing mode</strong>{item.actionPolicy === "approve_each" ? "Review every post" : item.actionPolicy.replaceAll("_", " ")}</p><p><strong className="block text-[#191714]">Safety switch</strong>{item.killSwitch ? "Publishing blocked" : "Approved schedules enabled"}</p></div></article>)}<div className="flex flex-wrap gap-3">{connection.killSwitch || connection.actionPolicy === "draft_only" ? <button type="button" disabled={busy} onClick={() => void enableApprovedScheduling()} className={button}>{busy ? "Saving…" : "Enable approved scheduling"}</button> : null}<button type="button" disabled={busy} onClick={() => void connect()} className={button}>Add another Page</button><button type="button" disabled={busy} onClick={() => void disconnect()} className="min-h-11 text-sm font-bold text-[#8f301f] underline underline-offset-4 disabled:opacity-50">Disconnect all</button></div></div> : <div className="mt-5"><ol className="grid gap-3 text-sm sm:grid-cols-3">{["Sign in to Meta", "Choose one or more Pages", "Confirm linked Instagram accounts"].map((item, index) => <li key={item} className="rounded-xl border border-[#191714]/10 bg-[#f7f2e9] p-3"><span className="mr-2 font-bold text-[#d94326]">{index + 1}</span>{item}</li>)}</ol>{ready ? <button type="button" disabled={busy} onClick={() => void connect()} className={`${button} mt-5`}>{busy ? "Opening Meta…" : "Connect Facebook & Instagram"}</button> : <p className="mt-5 rounded-xl border border-[#a9341f]/20 bg-[#fff1ec] p-3 text-sm font-semibold text-[#8f301f]">Meta authorization is temporarily unavailable. Your workspace remains private.</p>}</div>}
